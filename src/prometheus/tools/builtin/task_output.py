"""Tool for reading task output logs."""
...
class TaskOutputToolInput(BaseModel):
    ...
    max_bytes: int = Field(default=12000, ge=1, le=100_000, description="Max bytes to return.")
...
class TaskOutputTool(BaseTool):
    ...
    name = "task_output"
    description = "Read the tail of a background task's output log."
    ...
    async def execute(self, arguments: TaskOutputToolInput, context: ToolExecutionContext) -> ToolResult:
        ...
        return ToolResult(output=output or "(no output yet)")
