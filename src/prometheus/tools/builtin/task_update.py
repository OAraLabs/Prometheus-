"""Tool for updating task metadata."""
...
class TaskUpdateToolInput(BaseModel):
    ...
    progress: int | None = Field(default=None, ge=0, le=100, description="Progress 0-100.")
    status_note: str | None = Field(default=None, description="Short human-readable note.")


class TaskUpdateTool(BaseTool):
    """Update task description, progress, or status note."""

    name = "task_update"
    description = "Update a task's description, progress percentage, or status note."
    input_model = TaskUpdateToolInput

    async def execute(self, arguments: TaskUpdateToolInput, context: ToolExecutionContext) -> ToolResult:
        del context
        try:
